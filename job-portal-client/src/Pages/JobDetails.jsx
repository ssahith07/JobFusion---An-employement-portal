...
const JobDetails = () => {
  ...
  const app = applied.find(item => item === _id);
  // console.log(app);
      ...
      {sect === 'private' ? (<section className="max-w-screen-2xl container mx-auto xl:px-24 px-4 mt-10">
        <Link to={`/job-details/${_id}`} className="flex gap-4 flex-col sm:flex-row items-start">
          ...
          <div>
            ...
            {!app?(<button className="bg-green px-8 py-2 my-2 text-white rounded-full" onClick={handleApply}>
          Apply Now
        </button>):(<><button disabled={true} className="bg-gray-500 px-8 py-2 my-2 text-white rounded-full" >
          Applied 
        </button>
        <span className='px-4 font-normal text-gray-500'>* if you are selected you will receive email or receive call to mobile number.</span>
        </>
        )}
